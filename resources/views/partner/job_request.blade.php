                    ...
                    <div class="card card-docs mb-2">
                        <div class="p-10">
                            ...
                            <h1 class="anchor fw-bolder mb-5" id="zero-configuration">
                                <a href="javascript:;"></a>Jobs Requests
                            </h1>
                            ...
                            <div class="d-flex align-items-center rounded py-5 px-4 bg-light-primary">
                                ...
                                <!--end::Icon-->
                                <!--begin::Description-->
                                <div class="text-gray-700 fw-bold fs-6 lh-lg">Here we have a list of all of the Jobs Requests that we have.</div>
                                <!--end::Description-->
                            </div>
                            <!--end::Notice-->
                            <!--end::Heading-->
                        ...
                        <div class="card-header border-0">
                            ...
                            <div class="card-toolbar">
                                ...
                                <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                                    <!--begin::Admins-->
                                    <!-- <button type="button" class="btn btn-primary me-2" data-bs-toggle="modal" data-bs-target="#kt_modal_add_admins">
                            ...
                            <div class="py-5">
                                <table class="kt_datatable_example_1 table table-row-bordered gy-5">
                                    ...
                                    <tbody class="fw-bold text-gray-600">
                                        @for ($i = 0; $i < count($jobConfirmation); $i++) @php $a=$i; $a++; @endphp 
                                        <tr>
                                            <td>{{$a}}</td>
                                            <td>{{date("Y-m-d",strtotime($jobConfirmation[$i]->created_at))}}</td>
                                            <td>{{$jobConfirmation[$i]->position_apply_for}}</td>
                                            <td>{{$jobConfirmation[$i]->job_post_number}}</td>
                                            <td>{{$jobConfirmation[$i]->first_name}}</td>
                                            <td>{{$jobConfirmation[$i]->last_name}}</td>
                                            <td>{{$jobConfirmation[$i]->middle_name}}</td>
                                            <td>{{$jobConfirmation[$i]->telephone}}</td>
                                            <td>{{$jobConfirmation[$i]->e_mail_address}}</td>
                                            <td>{{$jobConfirmation[$i]->current_address}}</td>
                                            ...
                                            <td>
                                                <div class="badge badge-sm badge-light-primary d-inline cursor-pointer" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">{{$jobConfirmation[$i]->status}}</div>
                                                ...
                                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg-light-primary fw-bold w-200px py-3" data-kt-menu="true">
                                                    ...
                                                    <div class="menu-item px-3">
                                                        <a class="menu-link px-3" onclick="changeJobStatus(this,'Sent Email','{{$jobConfirmation[$i]->id}}')">Sent Email</a>
                                                    </div>
                                                    <div class="menu-item px-3">
                                                        <a class="menu-link px-3" onclick="changeJobStatus(this,'Keep','{{$jobConfirmation[$i]->id}}')">Keep</a>
                                                    </div>
                                                    <div class="menu-item px-3">
                                                        <a class="menu-link px-3" onclick="changeJobStatus(this,'Delete','{{$jobConfirmation[$i]->id}}')">Delete</a>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                <button class="btn btn-icon btn-sm btn-color-gray-400 btn-active-icon-primary me-2"  data-bs-original-title="Edit Job" onclick="editJobRequest('{{$jobConfirmation[$i]->id}}')">
                                                    <span class="svg-icon svg-icon-2">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
...
<script>
    ...
    var value = {
        ...
        $.ajax({
            type: 'GET',
            url: "{{ route('job_request_edit') }}",
            data: value,
            success: function(result) {
                $('#myModalLgHeading').html('Edit Job Request');
                $('#modalBodyLarge').html(result);
                $('#myModalLg').modal('show');
            }
        });
        }
    function changeJobStatus(obj,value,id)
    {   
        obj.parentElement.parentElement.parentElement.children[0].textContent=value;
        var value = {
            id:id,
            value:value,
        };
